...
import { Home, Activity, Settings, BarChart3, Bell, Database, Cpu } from 'lucide-react';
import PropTypes from 'prop-types';

const Sidebar = ({ currentView = 'dashboard', onViewChange }) => {
  const menuItems = [
  ...
  return (
    <aside className="w-20 lg:w-64 bg-surface/95 border-r border-white/5 flex flex-col backdrop-blur-xl z-50 transition-all duration-300">
      {/* Logo/Brand */}
      <div className="h-20 flex items-center justify-center lg:justify-start lg:px-6 border-b border-white/5 relative overflow-hidden">
        ...
        {menuItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`
                w-full flex items-center gap-4 px-3 lg:px-4 py-3 rounded-lg transition-all duration-300 group relative overflow-hidden
      ...
      <div className="p-4 border-t border-white/5">
        <div className="bg-surface-highlight/50 rounded-lg p-3 border border-white/5">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-xs font-mono text-slate-400 hidden lg:block">SYSTEM ONLINE</span>
          </div>
          <div className="hidden lg:block w-full bg-slate-800 h-1 rounded-full overflow-hidden">
